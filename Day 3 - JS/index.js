function calculateFinalScore(exam, project) {
  ...
  const finalScore = 0.6 * exam + 0.4 * project;
  return finalScore;
}
...
function getGrade(exam, project, absent) {
  if (isAttendanceEnough(absent)) {
    ...
    if (finalScore >= 70 && finalScore < 80 /* final score more or equal 70*/) {
      return "B";
    }

    if (finalScore >= 60 && finalScore < 70 /* final score more or equal 60*/) {
      return "C";
    }
